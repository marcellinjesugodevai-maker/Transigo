...
export default function DeliveryDetailPage() {
    ...
    const handleVerify = async () => {
        if (!confirm('Valider ce livreur ? Il pourra alors utiliser l\'application.')) return;
        ...
        if (error) {
            ...
            alert('Livreur validé avec succès !');
            fetchDriver();
        }
    ...
    const handleReject = async () => {
        if (!confirm('Rejeter ce livreur ? Son compte sera bloqué.')) return;
    ...
    if (error || !driver) {
        return (
            <div style={{ padding: 32, color: '#fff', textAlign: 'center' }}>
                <p style={{ color: '#ef4444' }}>Erreur: {error || 'Livreur non trouvé'}</p>
                <Link href="/deliveries" style={{ color: '#3b82f6' }}>← Retour</Link>
            </div>
        );
    }

    const documents = [
        { key: 'id_card_url', label: 'CNI / Document ID', url: driver.id_card_url },
        { key: 'license_front_url', label: 'Permis/Doc (Recto)', url: driver.license_front_url },
        { key: 'license_back_url', label: 'Permis/Doc (Verso)', url: driver.license_back_url },
        { key: 'registration_card_url', label: 'Carte Grise (si applic.)', url: driver.registration_card_url },
    ];
    ...
    return (
        <div style={{ padding: 32, color: '#fff', maxWidth: 1000, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
                <Link href="/deliveries" style={{ color: '#94a3b8', fontSize: 24, textDecoration: 'none' }}>←</Link>
                <div>
                    <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700 }}>
                        📦 {driver.first_name} {driver.last_name}
                    </h1>
                    <p style={{ margin: '8px 0 0', color: '#94a3b8' }}>
                        Inscrit en tant que Livreur le {new Date(driver.created_at).toLocaleDateString('fr-FR')}
                    </p>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 24 }}>
                ...
                {/* Vehicle */}
                <div style={cardStyle}>
                    <h3 style={{ margin: '0 0 16px', fontSize: 16, color: '#94a3b8' }}>🛵 Véhicule de livraison</h3>
                    {driver.vehicle_type ? (
                        <>
                            <p style={{ margin: '8px 0' }}><strong>Type:</strong> {driver.vehicle_type}</p>
                            <p style={{ margin: '8px 0' }}><strong>Marque/Modèle:</strong> {driver.vehicle_brand || '-'} {driver.vehicle_model || '-'}</p>
                            <p style={{ margin: '8px 0' }}><strong>Plaque:</strong> {driver.vehicle_plate || 'N/A'}</p>
                        </>
                    ) : (
                        <p style={{ color: '#f59e0b' }}>⚠️ Type non précisé</p>
                    )}
                </div>
                ...
                <div style={cardStyle}>
                    ...
                    <p style={{ margin: '8px 0' }}><strong>Livraisons:</strong> {driver.total_rides || 0}</p>
