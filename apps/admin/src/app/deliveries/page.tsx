...
export default function DeliveriesPage() {
    ...
    const [filter, setFilter] = useState('delivery'); // Default to delivery
    ...
    const handleBlockToggle = async (driver: Driver) => {
        if (!confirm(`Voulez-vous vraiment ${driver.is_blocked ? 'débloquer' : 'bloquer'} ce livreur ?`)) return;
    ...
    const filteredDrivers = drivers.filter(d => {
        // Force delivery profile in this view
        if (d.profile_type !== 'delivery') return false;

        const matchesFilter =
            filter === 'all' || filter === 'delivery' ? true :
                filter === 'online' ? d.is_online :
                    filter === 'blocked' ? d.is_blocked :
                        filter === 'pending' ? !d.is_verified : true;
    ...
    return (
        <div style={{ padding: 32, color: '#fff' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <h1 style={{ margin: 0, fontSize: 32, fontWeight: 700 }}>🛵 Gestion Livreurs</h1>
                    </div>
                    <p style={{ margin: '8px 0 0 40px', color: '#94a3b8' }}>
                        {filteredDrivers.length} livreurs enregistrés
                    </p>
                </div>
                <button
            ...
            <div style={{ marginBottom: 24, display: 'flex', gap: 16 }}>
                ...
                >
                    <option value="delivery">Tous les livreurs</option>
            ...
            {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>Aucun livreur trouvé</div>
            ) : (
                <div style={{ display: 'grid', gap: 16 }}>
                    {filteredDrivers.map(driver => (
                        <div key={driver.id} style={{ ...cardStyle, cursor: 'pointer' }} onClick={() => window.location.href = `/deliveries/${driver.id}`}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ color: '#94a3b8', fontSize: 13, marginTop: 4 }}>
                                            📦 {driver.phone} • ⭐ {driver.rating?.toFixed(1) || '5.0'} • {driver.total_rides || 0} livraisons
                                        </div>
                                    </div>
                                </div>
